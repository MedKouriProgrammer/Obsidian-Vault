---
creation_date: 2025-10-30T14:28
tags:
  - Resources/Articles
Resource: https://www.youtube.com/watch?v=ZGizsqwe4ps&list=PLDoPjvoNmBAyE_gei5d18qkfIe-Z8mocs&index=101
Author: "[[Alzero Wep School]]"
topic: "[[101 - Regular Expressions Part 7 Re Module Split And Sub]]"
---
> [!summary]-
> 

# <font color="#646a73">101 - Regular Expressions Part 7 Re Module Split And Sub</font>
---
## <font color="#ffc000">Explanation and method of use:</font>
#### Input:
```python
# ----------------------------------------------------
# -- Regular Expressions => Re Module Split And Sub --
# ----------------------------------------------------
# split(Pattern, String, MaxSplit)  => Return A List Of Elements Splitted On Each Match
# sub(Pattern, Replace, String, ReplaceCount) => Replace Matches With What You Want
# ---------------------------------------------------------------------

import re

string_one = "I Love Python Programming Language"

search_one = re.split(r"\s", string_one, 1)

print(search_one)

print("#" * 50)

string_two = "How-To_Write_A_Very-Good-Article"

search_two = re.split(r"-|_", string_two)

print(search_two)

print("#" * 50)

# Get Words From URL

for counter, word in enumerate(search_two, 1):

  if len(word) == 1:

    continue

  print(f"Word Number: {counter} => {word.lower()}")

print("#" * 50)

my_string = "I Love Python"

print(re.sub(r"\s", "-", my_string, 1))
```
Output:
```
 search_one = re.split(r"\s", string_one, 1)
['I', 'Love Python Programming Language']
##################################################    
['How', 'To', 'Write', 'A', 'Very', 'Good', 'Article']
##################################################    
Word Number: 1 => how
Word Number: 2 => to
Word Number: 3 => write
Word Number: 5 => very
Word Number: 6 => good
Word Number: 7 => article
##################################################
c:\Users\ad\Documents\Cemder\MedDir\file.py:40: DeprecationWarning: 'count' is passed as positional argument
  print(re.sub(r"\s", "-", my_string, 1))
I-Love Python
```